...
def process_excel(file_path):
    ...
    # Aplicar fórmula para a Coluna A (valor do arquivo de entrada + 0)
    ...
    # Selecionar apenas as colunas desejadas para o arquivo de saída
    df_output = df[['Circunferência', 'Diâmetro', 'Partes perenes acima e abaixo do solo mais folhas', 'Biomassa C (kg)', 'CO2eq']]

    # Salvar o arquivo Excel processado
    output_file = file_path.replace('.xlsx', '_Calculado.xlsx')
    df_output.to_excel(output_file, index=False)

    # Ajustar a largura das células e adicionar bordas
    wb = load_workbook(output_file)
    ws = wb.active
    adjust_column_width(ws)
    add_borders(ws)

    # Adicionar cor verde intermediária à linha do cabeçalho (linha 1) e à linha de média (última linha)
    ...
    # Gerar gráfico
    num_plants = len(df) - 1  # Excluindo a linha de média
    ...
    plt.subplots_adjust(left=0.1, right=0.9, top=0.9, bottom=0.3)  # Ajuste das margens
    plt.savefig(file_path.replace('.xlsx', '_Graph.png'))  # Salvar o gráfico como uma imagem
...
root.geometry("580x200")  # Aumenta o tamanho do painel
root.mainloop()
